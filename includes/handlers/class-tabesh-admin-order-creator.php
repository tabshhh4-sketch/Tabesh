...
class Tabesh_Admin_Order_Creator {
	...
	public function rest_create_order( $request ) {
		...
		$wpdb->insert(
			...
			array(
				...
				'description' => sprintf(
					__( 'سفارش #%1$s برای کاربر %2$s توسط مدیر ایجاد شد', 'tabesh' ),
					$order_number,
					$user->display_name
				),
			)
	...
	public function render_new_order_button() {
		...
		if ( $is_admin_context ) {
			...
			// Frontend styling to match dashboard header buttons
			?>
			<button type="button" class="header-btn" id="tabesh-open-order-modal">
				➕ <span><?php echo esc_html__( 'ثبت سفارش جدید', 'tabesh' ); ?></span>
			</button>
			<?php
		}
	}
