...
class Tabesh_Order {
    ...
    /**
     * Get printing sub-status structure
     * 
     * Defines the sub-statuses for the printing workflow.
     * Each sub-status has a key, label, and field to extract from order data.
     * 
     * @return array Sub-status definitions
     */
    public function get_printing_sub_statuses() {
        return array(
            'cover_printing' => array(
                'label' => 'چاپ جلد',
                'order_field' => 'cover_paper_weight',
                'order_field_label' => 'گرماژ کاغذ جلد'
            ),
            'cover_lamination' => array(
                'label' => 'سلفون جلد',
                'order_field' => 'lamination_type',
                'order_field_label' => 'نوع سلفون'
            ),
            'text_printing' => array(
                'label' => 'چاپ متن کتاب',
                'order_field' => 'paper_type',
                'order_field_label' => 'نوع و گرماژ کاغذ',
                'order_field_extra' => 'paper_weight'
            ),
            'binding' => array(
                'label' => 'صحافی',
                'order_field' => 'binding_type',
                'order_field_label' => 'نوع صحافی'
            ),
            'additional_services' => array(
                'label' => 'خدمات اضافی',
                'order_field' => 'extras',
                'order_field_label' => 'خدمات',
                'optional' => true // Only show if extras exist
            )
        );
    }

    /**
     * Initialize sub-statuses for a printing order
     * 
     * Creates the default sub-status structure with all items unchecked.
     * 
     * @param int $order_id Order ID
     * @return array|false Sub-status data or false on failure
     */
    public function initialize_sub_statuses($order_id) {
        global $wpdb;
        $table = $wpdb->prefix . 'tabesh_orders';
        
        // Get order to check if extras exist
        $order = $wpdb->get_row($wpdb->prepare(
            "SELECT extras FROM $table WHERE id = %d",
            $order_id
        ));
        ...
        $sub_statuses = $this->get_printing_sub_statuses();
        $initialized = array();
    ...
    /**
     * Get order sub-status data
     * 
     * Retrieves and parses sub-status JSON from order record.
     * 
     * @param int $order_id Order ID
     * @return array|null Sub-status data or null if not set
     */
    public function get_order_sub_status_data($order_id) {
        global $wpdb;
        $table = $wpdb->prefix . 'tabesh_orders';
        
        $sub_statuses_json = $wpdb->get_var($wpdb->prepare(
            "SELECT sub_statuses FROM $table WHERE id = %d",
            $order_id
        ));
        
        if (empty($sub_statuses_json)) {
            return null;
        }
    ...
    /**
     * Update sub-status completion state
     * 
     * Updates a specific sub-status and stores who completed it and when.
     * 
     * @param int $order_id Order ID
     * @param string $sub_status_key Sub-status key
     * @param bool $completed Completion state
     * @return bool Success
     */
    public function update_sub_status($order_id, $sub_status_key, $completed) {
        global $wpdb;
        $table = $wpdb->prefix . 'tabesh_orders';
        
        // Get current sub-status data
        $current_data = $this->get_order_sub_status_data($order_id);
        
        // Initialize if doesn't exist
        if ($current_data === null) {
            $current_data = $this->initialize_sub_statuses($order_id);
        }
        
        // Validate sub-status key
        $valid_keys = array_keys($this->get_printing_sub_statuses());
        if (!in_array($sub_status_key, $valid_keys)) {
            return false;
        }
        
        // Update the specific sub-status
        if (!isset($current_data[$sub_status_key])) {
            $current_data[$sub_status_key] = array();
        }
        
        $current_data[$sub_status_key]['completed'] = (bool)$completed;
        
        if ($completed) {
            $current_data[$sub_status_key]['completed_at'] = current_time('mysql');
            $current_data[$sub_status_key]['completed_by'] = get_current_user_id();
        } else {
            $current_data[$sub_status_key]['completed_at'] = null;
            $current_data[$sub_status_key]['completed_by'] = null;
        }
        
        // Save updated data
        $result = $wpdb->update(
            $table,
            array('sub_statuses' => wp_json_encode($current_data)),
            array('id' => $order_id),
            array('%s'),
            array('%d')
        );
    ...
    /**
     * Calculate sub-status completion percentage
     * 
     * Returns the percentage of completed sub-statuses.
     * 
     * @param int $order_id Order ID
     * @return int Completion percentage (0-100)
     */
    public function get_sub_status_completion($order_id) {
        $data = $this->get_order_sub_status_data($order_id);
        ...
        $completed = 0;
        foreach ($data as $sub_status) {
            if (isset($sub_status['completed']) && $sub_status['completed']) {
                $completed++;
            }
        }
     ...
     * 
     * Used to determine if the main printing status should be auto-completed.
     * 
     * @param int $order_id Order ID
     * @return bool True if all sub-statuses completed
     */
    public function should_auto_complete_printing($order_id) {
        $data = $this->get_order_sub_status_data($order_id);
