...
final class Tabesh {
    ...
    private function update_database_schema() {
        ...
        $transfer_status_exists = $wpdb->get_var($wpdb->prepare(
        ...
        if (!$transfer_status_exists) {
            ...
            $expires_at_exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM information_schema.columns 
        ...
        $scheduled_deletion_exists = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM information_schema.columns 
        ...
        if (!$scheduled_deletion_exists) {
            ...
            $transferred_at_exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM information_schema.columns 
        ...
        $orders_table_exists = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = %s AND table_name = %s",
            DB_NAME,
            $table_orders
        ));
        ...
        if ($orders_table_exists) {
            ...
            $book_title_exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM information_schema.columns 
            ...
            if (!$book_title_exists) {
                ...
                $order_number_exists = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(*) FROM information_schema.columns 
    ...
    private function set_default_options() {
        $defaults = array(
            ...
            'paper_types' => json_encode(array('تحریر' => array(60, 70, 80), 'بالک' => array(60, 70, 80, 100))),
            ...
            'pricing_paper_types' => json_encode(array(
                'glossy' => 250,
                'matte' => 200,
                'cream' => 180,
                'تحریر' => 200,
            ...
            'pricing_print_costs' => json_encode(array(
                'bw' => 200,
                'color' => 800,
            )),
            'pricing_cover_types' => json_encode(array(
                'soft' => 8000,
                'hard' => 15000,
            )),
            'pricing_lamination_costs' => json_encode(array(
                'براق' => 2000,
                'مات' => 2500,
                'بدون سلفون' => 0,
            )),
            'pricing_binding_costs' => json_encode(array(
                'شومیز' => 3000,
                'جلد سخت' => 8000,
                'گالینگور' => 6000,
                'سیمی' => 2000,
            )),
            'pricing_options_costs' => json_encode(array(
                'لب گرد' => 1000,
                'خط تا' => 500,
                'شیرینک' => 1500,
                'سوراخ' => 300,
    ...
    private function set_default_format_settings() {
        ...
        $format_settings = array(
            ...
            array(
                ...
                'validation_rules' => json_encode(array('check_margins' => true, 'check_page_count' => true))
            ),
            array(
                'book_format' => 'A5',
                ...
                'validation_rules' => json_encode(array('check_bleed' => true, 'check_color_mode' => true))
            ),
    ...
    public function register_rest_routes() {
        register_rest_route(TABESH_REST_NAMESPACE, '/calculate-price', array(
            'methods' => 'POST',
        ...
        register_rest_route(TABESH_REST_NAMESPACE, '/update-status', array(
            'methods' => 'POST',
        ...
        register_rest_route(TABESH_REST_NAMESPACE, '/staff/update-status', array(
            'methods' => 'POST',
            'callback' => array($this->staff, 'update_status_rest'),
            'permission_callback' => array($this, 'can_manage_orders')
        ));
        ...
        register_rest_route(TABESH_REST_NAMESPACE, '/user-orders/search', array(
            'methods' => 'GET',
            'callback' => array($this->user, 'search_user_orders'),
            'permission_callback' => array($this, 'is_user_logged_in')
        ));
        
        register_rest_route(TABESH_REST_NAMESPACE, '/user-orders/summary', array(
            'methods' => 'GET',
            'callback' => array($this->user, 'get_orders_summary'),
            'permission_callback' => array($this, 'is_user_logged_in')
        ));
        
        register_rest_route(TABESH_REST_NAMESPACE, '/user-orders/(?P<order_id>\d+)', array(
            'methods' => 'GET',
