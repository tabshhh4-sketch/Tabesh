...
final class Tabesh {
	...
	public function register_rest_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'rest_get_ftp_status' ),
				'permission_callback' => array( $this, 'can_manage_admin' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_refresh_ftp_status' ),
				'permission_callback' => array( $this, 'can_manage_admin' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_archive_order' ),
				'permission_callback' => array( $this, 'can_manage_admin' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_restore_order' ),
				'permission_callback' => array( $this, 'can_manage_admin' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'rest_get_print_substeps' ),
				'permission_callback' => array( $this, 'can_manage_orders' ),
			)
		);
	...
	/**
	 * REST: Get orders list for React dashboard
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_get_orders( $request ) {
		...
		$search   = sanitize_text_field( $request->get_param( 'customer' ) ?? '' );
		$sort_by  = sanitize_text_field( $request->get_param( 'sortBy' ) ?? 'newest' );

		// Get orders from admin handler.
		$all_orders = $this->admin->get_orders( $search, false );

		// Filter by status.
		if ( ! empty( $status ) ) {
			$all_orders = array_filter(
				$all_orders,
		...
		foreach ( $orders as $order ) {
			...
			$formatted_orders[] = array(
				'id'               => $order->id,
				'serial_number'    => $order->serial_number ?? 0,
	...
	public function rest_get_order( $request ) {
		...
		if ( ! $order ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'error'   => __( 'سفارش یافت نشد', 'tabesh' ),
				),
				404
			);
		}
	...
	/**
	 * REST: Get dashboard statistics
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_get_statistics( $request ) {
		$stats = $this->admin->get_statistics();
	 ...
	 * @return WP_REST_Response Response object.
	 */
	public function rest_get_ftp_status( $request ) {
		if ( ! isset( $this->ftp_handler ) ) {
			return new WP_REST_Response(
				array(
					'connected' => false,
					'message'   => __( 'FTP handler not available', 'tabesh' ),
				),
				200
			);
		}

		$status = $this->ftp_handler->get_connection_status();
	...
	/**
	 * REST: Refresh FTP status
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_refresh_ftp_status( $request ) {
		if ( ! isset( $this->ftp_handler ) ) {
			return new WP_REST_Response(
				array(
					'connected' => false,
		...
		$status = $this->ftp_handler->check_connection();
	...
	/**
	 * REST: Archive an order
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_archive_order( $request ) {
		$order_id = intval( $request->get_param( 'order_id' ) );
		...
		if ( ! $order_id ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'error'   => __( 'شناسه سفارش نامعتبر است', 'tabesh' ),
				),
				400
			);
		}

		$result = $this->archive->archive_order( $order_id );
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'سفارش با موفقیت آرشیو شد', 'tabesh' ),
			),
			200
		);
	}
	...
	/**
	 * REST: Restore an archived order
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_restore_order( $request ) {
		$order_id = intval( $request->get_param( 'order_id' ) );
		...
		$result = $this->archive->restore_order( $order_id );
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'سفارش با موفقیت بازیابی شد', 'tabesh' ),
			),
			200
		);
	}
	...
	/**
	 * REST: Get print substeps for an order
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return WP_REST_Response Response object.
	 */
	public function rest_get_print_substeps( $request ) {
		$order_id = intval( $request->get_param( 'order_id' ) );
		...
		if ( ! isset( $this->print_substeps ) ) {
			return new WP_REST_Response( array(), 200 );
		}

		$substeps = $this->print_substeps->get_order_substeps( $order_id );
		...
		foreach ( $substeps as $substep ) {
			$formatted_substeps[] = array(
				...
				'substep_key'   => $substep->substep_key,
				'substep_label' => $substep->substep_title,
				'is_completed'  => ! empty( $substep->is_completed ),
				'completed_at'  => $substep->completed_at ?? null,
				'completed_by'  => $substep->completed_by ?? null,
			);
		}

		return new WP_REST_Response( $formatted_substeps, 200 );
	}
}
